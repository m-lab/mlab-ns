...
def get_nagios_credentials(): 
    ...
    if not nagios: 
        nagios = list(model.Nagios.get_by_key_name(constants.DEFAULT_NAGIOS_ENTRY))
        if nagios:
...
def get_tools_by_id(): 
	...
	if not tools: 
		tools = list(model.Tool.gql('ORDER by tool_id DESC').run(batch_size=constants.GQL_BATCH_SIZE))
...
def get_SliverTool_by_tool_id(tool_id): 
	...
	if not sliver_tools: 
		sliver_tools= list(model.SliverTool.gql('WHERE tool_id=:tool_id', tool_id=tool_id).run(batch_size=constants.GQL_BATCH_SIZE))
