...
class GetSliceUrlTest(unittest2.TestCase):
	...
	def test_get_urls_successfully(self):
		expected_urls= [('http://nagios.measurementlab.net/baseList?show_state=1&service_name=mock_one_ipv6&plugin_output=1', 'mock_one', '_ipv6'),
						('http://nagios.measurementlab.net/baseList?show_state=1&service_name=mock_two_ipv6&plugin_output=1', 'mock_two', '_ipv6')]

		with mock.patch('mlabns.util.nagios_status.nagios_status_data') as mock_nagios:
			mock_nagios.get_tools_by_id.return_value= [mock.Mock(tool_id='mock_one'), mock.Mock(tool_id='mock_two')]
			actual_urls= nagios_status.get_slice_urls(self.nagios_url, self.nagios_suffixes)
...
class ParseSliverToolStatusTest(unittest2.TestCase):
	...
	def test_successful_parsing(self): 
		...
		expected_parsed_status= ['ndt.iupui.mlab1.acc02.measurement-lab.org', '0','mock tool extra']
...
class EvaluateStatusUpdateTest(unittest2.TestCase):
	...
	def test_version4_has_no_ip_and_was_online(self): 
		with mock.patch.object(nagios_status, 'was_online', return_value=True): 
			with mock.patch.object(nagios_status, 'has_no_ip', return_value=True): 
				mock_sliver_tool= mock.Mock(fqdn='mock_fqdn')
				nagios_status.evaluate_status_update(mock_sliver_tool, self.four, self.slice_status(self.online))
	...
	def test_version6_has_ip_and_no_change(self): 
		with mock.patch.object(nagios_status, 'has_no_ip', return_value=False): 
			mock_sliver_tool= mock.Mock(fqdn='mock_fqdn')
			nagios_status.evaluate_status_update(mock_sliver_tool, self.six, self.slice_status(self.online))
...
class GetSliceStatusTest(unittest2.TestCase):
	...
	def test_http_error(self): 
		with mock.patch.object(urllib2, 'urlopen', side_effect=MockHttpError('mock_code')):
	...
	def test_one_none_status(self):
		with mock.patch.object(urllib2, 'urlopen'): 
			with mock.patch.object(nagios_status, 'parse_sliver_tool_status', return_value=None):
...
class UpdateSliverToolsStatusTest(unittest2.TestCase):
	...
	def setUp(self): 
		evaluate_patch= mock.patch.object(nagios_status, 'evaluate_status_update', autospec=True)
	...
	def test_db_error_continues(self): 
		...
		with mock.patch.object(nagios_status_data, 'get_SliverTool_by_tool_id', return_value=slivers):
			nagios_status.update_sliver_tools_status({'mock_fqdn':'mock_status'}, 'mock_tool_id', 'mock_ip')
	...
	def test_everything_successful(self): 
		...
		with mock.patch.object(nagios_status_data, 'get_SliverTool_by_tool_id', return_value=slivers):
			nagios_status.update_sliver_tools_status('mock_status', 'mock_tool_id', 'mock_ip')
