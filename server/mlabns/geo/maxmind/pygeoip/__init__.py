...
MMAP_CACHE = constant.MMAP_CACHE
MEMORY_CACHE = constant.MEMORY_CACHE
...
class GeoIP(GeoIPBase):
    ...
    def __init__(self, filename, flags=0):
        ...
        if self._flags & constant.MMAP_CACHE:
            with open(filename, 'rb') as f:
                self._filehandle = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)

        elif self._flags & constant.MEMORY_CACHE:
    ...
    def _setup_segments(self):
        ...
        """
        self._databaseType = constant.COUNTRY_EDITION
        self._recordLength = constant.STANDARD_RECORD_LENGTH

        filepos = self._filehandle.tell()
        self._filehandle.seek(-3, os.SEEK_END)

        for i in range(constant.STRUCTURE_INFO_MAX_SIZE):
            delim = self._filehandle.read(3)

            if delim == sixx.u(chr(255) * 3):
                ...
                if self._databaseType == constant.REGION_EDITION_REV0:
                    self._databaseSegments = constant.STATE_BEGIN_REV0

                elif self._databaseType == constant.REGION_EDITION_REV1:
                    self._databaseSegments = constant.STATE_BEGIN_REV1

                elif self._databaseType in (constant.CITY_EDITION_REV0,
                                            constant.CITY_EDITION_REV1,
                                            constant.ORG_EDITION,
                                            constant.ISP_EDITION,
                                            constant.ASNUM_EDITION):
                    self._databaseSegments = 0
                    buf = self._filehandle.read(constant.SEGMENT_RECORD_LENGTH)

                    for j in range(constant.SEGMENT_RECORD_LENGTH):
                        self._databaseSegments += (ord(buf[j]) << (j * 8))

                    if self._databaseType in (constant.ORG_EDITION, constant.ISP_EDITION):
                        self._recordLength = constant.ORG_RECORD_LENGTH

                break
            else:
                self._filehandle.seek(-4, os.SEEK_CUR)

        if self._databaseType == constant.COUNTRY_EDITION:
            self._databaseSegments = constant.COUNTRY_BEGIN
    ...
    def _lookup_country_id(self, addr):
        ...
        return self._seek_country(ipnum) - constant.COUNTRY_BEGIN
    ...
    def _seek_country(self, ipnum):
        ...
        for depth in range(31, -1, -1):
            ...
            if self._flags & constant.MEMORY_CACHE:
    ...
    def _get_org(self, ipnum):
        ...
        org_buf = self._filehandle.read(constant.MAX_ORG_RECORD_LENGTH)
    ...
    def _get_region(self, ipnum):
        ...
        if self._databaseType == constant.REGION_EDITION_REV0:
            ...
            seek_region = seek_country - constant.STATE_BEGIN_REV0
            if seek_region >= 1000:
                country_code = 'US'
                region = ''.join([chr((seek_region // 1000) // 26 + 65), chr((seek_region // 1000) % 26 + 65)])
            else:
                country_code = constant.COUNTRY_CODES[seek_region]
                region = ''
        elif self._databaseType == constant.REGION_EDITION_REV1:
            seek_country = self._seek_country(ipnum)
            seek_region = seek_country - constant.STATE_BEGIN_REV1
            if seek_region < constant.US_OFFSET:
                country_code = '';
                region = ''
            elif seek_region < constant.CANADA_OFFSET:
                country_code = 'US'
                region = ''.join([chr((seek_region - constant.US_OFFSET) // 26 + 65), chr((seek_region - constant.US_OFFSET) % 26 + 65)])
            elif seek_region  < constant.WORLD_OFFSET:
                country_code = 'CA'
                region = ''.join([chr((seek_region - constant.CANADA_OFFSET) // 26 + 65), chr((seek_region - constant.CANADA_OFFSET) % 26 + 65)])
            else:
                i = (seek_region - constant.WORLD_OFFSET) // constant.FIPS_RANGE
                if i < len(constant.COUNTRY_CODES):
                    #country_code = constant.COUNTRY_CODES[(seek_region - constant.WORLD_OFFSET) // constant.FIPS_RANGE]
                    country_code = constant.COUNTRY_CODES[i]
                else:
                    country_code = ''
                region = ''

        elif self._databaseType in (constant.CITY_EDITION_REV0, constant.CITY_EDITION_REV1):
    ...
    def _get_record(self, ipnum):
        ...
        self._filehandle.seek(record_pointer, os.SEEK_SET)
        record_buf = self._filehandle.read(constant.FULL_RECORD_LENGTH)

        record = {}

        record_buf_pos = 0
        char = ord(record_buf[record_buf_pos])
        #char = record_buf[record_buf_pos] if sixx.PY3 else ord(record_buf[record_buf_pos])
        record['country_code'] = constant.COUNTRY_CODES[char]
        record['country_code3'] = constant.COUNTRY_CODES3[char]
        record['country_name'] = constant.COUNTRY_NAMES[char]
        record_buf_pos += 1
        str_length = 0
        ...
        if self._databaseType == constant.CITY_EDITION_REV1:
            dmaarea_combo = 0
        ...
        if 'dma_code' in record and record['dma_code'] in constant.DMA_MAP:
            record['metro_code'] = constant.DMA_MAP[record['dma_code']]
        else:
            record['metro_code'] = ''
    ...
    def country_code_by_addr(self, addr):
        ...
        try:
            if self._databaseType == constant.COUNTRY_EDITION:
                ...
                return constant.COUNTRY_CODES[country_id]
            elif self._databaseType in (constant.REGION_EDITION_REV0, constant.REGION_EDITION_REV1,
                                          constant.CITY_EDITION_REV0, constant.CITY_EDITION_REV1):
    ...
    def country_name_by_addr(self, addr):
        ...
        try:
            if self._databaseType == constant.COUNTRY_EDITION:
                country_id = self._lookup_country_id(addr)
                return constant.COUNTRY_NAMES[country_id]
            elif self._databaseType in (constant.CITY_EDITION_REV0, constant.CITY_EDITION_REV1):
    ...
    def org_by_addr(self, addr):
        ...
        try:
            ...
            if self._databaseType not in (constant.ORG_EDITION, constant.ISP_EDITION, constant.ASNUM_EDITION):
    ...
    def record_by_addr(self, addr):
        ...
        try:
            ...
            if not self._databaseType in (constant.CITY_EDITION_REV0, constant.CITY_EDITION_REV1):
    ...
    def region_by_addr(self, addr):
        ...
        try:
            ...
            if not self._databaseType in (constant.REGION_EDITION_REV0, constant.REGION_EDITION_REV1,
                                          constant.CITY_EDITION_REV0, constant.CITY_EDITION_REV1):
