...
# used in the CSV files and the conversion algorithm see
...
class GeoRecord:
    ...
    def __ne__(self, other):
        return ((self.city != other.city) or
...
def get_ip_geolocation(ip_address):
    ...
    try:
        """Code relies on modules for input validation, which throw Exceptions.

        Raises:
            socket.error: Generated by pygeoip in parsing addresses from test to
                binary form while searching the MaxMind database.
            TypeError: Passed a non-string IP address, such as None.
        """
    ...
    except (socket.error, TypeError) as exception_error:
        logging.error("MaxMind Geolocation failed on query '%s' with error: %s",
                ip_address, exception_error)
        return GeoRecord()
